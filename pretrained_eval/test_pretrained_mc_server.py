...
def load_id_ood_manifest(data_dir: Path, datasets: list[str]) -> dict[str, Any]:
    ...
    for pde in datasets:
        ...
        for condition in ID_OOD_CONDITIONS:
            ...
            if len(condition_entries) != 3:
                raise RuntimeError(
                    f"ID/OOD manifest must define 3 {condition} entries for {pde}."
                )
...
def validate_id_ood_test_split(
    ...
    if manifest_ids != split_info["selected_sim_ids"]:
        raise RuntimeError(
            f"ID/OOD test split for {pde} does not match manifest sim IDs."
        )
                ...
                writer.writerow(
                    {
                        ...
                        "count": result["count"],
                        **{
                            f"nRMSE_{k}": f"{result['macro'][k]:.6g}"
                            for k in args.eval_k
                        },
                    }
                )
                writer.writerow(
                    {
                        ...
                        "count": result["count"],
                        **{
                            f"nRMSE_{k}": f"{result['micro'][k]:.6g}"
                            for k in args.eval_k
                        },
                    }
                )
...
def main() -> None:
    ...
    if args.strict_official_test and args.id_ood_test:
        raise SystemExit(
            "--strict-official-test and --id-ood-test are mutually exclusive."
        )
    if args.strict_official_test:
    ...
    if args.id_ood_test:
        ...
        if len(args.datasets) != len(set(args.datasets)):
            raise SystemExit("--id-ood-test does not allow duplicate dataset names.")
        if args.test_unrolling_steps != 29 or args.rollout_steps != 30:
            raise SystemExit(
                "--id-ood-test requires test_unrolling_steps=29 and rollout_steps=30."
            )
        if args.max_batches_per_dataset is not None:
            raise SystemExit(
                "--id-ood-test requires all nine trajectories per PDE; "
                "remove --max-batches-per-dataset."
            )
        if args.downsample_factor != 2 or args.sample_size != 128:
            raise SystemExit(
                "--id-ood-test requires downsample_factor=2 and sample_size=128."
            )
    ...
    args.id_ood_manifest = (
        load_id_ood_manifest(data_dir, args.datasets) if args.id_ood_test else None
    )
    data_dir_warning = (
        "official" not in str(data_dir).lower() and not args.id_ood_test
    )
    if data_dir_warning:
        if args.strict_official_test:
            raise SystemExit(
