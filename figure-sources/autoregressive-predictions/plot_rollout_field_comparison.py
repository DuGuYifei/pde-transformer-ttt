...
def main() -> None:
    ...
    for pde in pdes:
        ...
        fig, axes = plt.subplots(
            ...
            gridspec_kw={"wspace": 0.012, "hspace": 0.022},
        )
        ...
        for row, (row_name, data) in enumerate(zip(row_names, row_data)):
            for col, step in enumerate(steps):
                ...
                ax.set_yticks([])
                if row == 0:
                    ax.set_title(f"Step {step}", pad=5, fontsize=18)
                if col == 0:
                    ax.set_ylabel(
                        row_name,
                        rotation=0,
        ...
        fig.suptitle(
            PDE_LABELS.get(pde, pde),
            y=1.07,
            fontsize=22,
            fontweight="bold",
        )
        colorbar = fig.colorbar(image, ax=axes, orientation="horizontal", fraction=0.025, pad=0.018)
