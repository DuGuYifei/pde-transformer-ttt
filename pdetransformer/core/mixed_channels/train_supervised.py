...
class SingleStepSupervised(lightning.LightningModule):
    def __init__(self,
                 ...
                 use_temporal_ttt_inference: bool | None = None,
                 ):
        ...
        self.use_temporal_ttt_inference = use_temporal_ttt_inference
    ...
    def predict_step(self, previous_frame,
        ...
        if "use_temporal_ttt" not in kwargs and self.use_temporal_ttt_inference is not None:
            kwargs["use_temporal_ttt"] = self.use_temporal_ttt_inference
        model_output = self.model(
            previous_frame,
...
class TemporalRolloutSupervised(SingleStepSupervised):
    """Autoregressive PDE training with persistent temporal TTT state and TBPTT."""
    ...
    def configure_optimizers(self):
        ...
        if not temporal_params:
            raise RuntimeError("TemporalRolloutSupervised requires trainable temporal_ttt parameters")

        temporal_lr = self.temporal_ttt_learning_rate or self.learning_rate
        parameter_groups = []
        if base_params:
            parameter_groups.append({"params": base_params, "lr": self.learning_rate})
        parameter_groups.append({"params": temporal_params, "lr": temporal_lr})
        return torch.optim.AdamW(parameter_groups, weight_decay=1e-15)
