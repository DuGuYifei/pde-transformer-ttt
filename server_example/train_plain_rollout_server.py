"""Fine-tune the clean Plain Attention model on a 29-step PDE rollout."""
...
def load_config(path: Path) -> dict:
    ...
    if config["token_mixer_type"] != "attention":
        raise ValueError("Plain rollout training requires token_mixer_type=attention")
    if config["carrier_token_active"]:
        raise ValueError("The matched clean Plain model does not use carrier tokens")
    ...
    if not config["init_checkpoint"]:
        raise ValueError("A trained clean Plain init_checkpoint is required")
    return config
...
def load_initial_weights(module: AutoregressiveRolloutSupervised, path: Path) -> None:
    ...
    if not path.is_file():
        raise FileNotFoundError(f"Plain initialization checkpoint does not exist: {path}")
    checkpoint = torch.load(path, map_location="cpu", weights_only=False)
    state_dict = checkpoint.get("state_dict", checkpoint)
    module.load_state_dict(state_dict, strict=True)
    print(f"initialized clean Plain from {path}")
...
def main() -> None:
    ...
    if total_parameters != trainable_parameters:
        raise RuntimeError(
            f"P-R29 expects full fine-tuning: total={total_parameters}, "
    ...
    print("internal temporal state: none (prediction graph only)")
