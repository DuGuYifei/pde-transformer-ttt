...
from pdetransformer.core.mixed_channels.pde_transformer import (  # noqa: E402
    WindowAttention2DTime,
)
from server_example.train_plain_rollout_server import (  # noqa: E402
    build_training_module,
    load_config,
)


CONFIG = ROOT / "server_example" / "pdes_attention-r29_128_100ep_60sims.yaml"
...
def test_config_and_model() -> None:
    ...
    assert config["token_mixer_type"] == "attention"
    assert "pdes_attention_128_60sims_100ep" in config["init_checkpoint"]
    assert "epoch-epoch=096.ckpt" in config["init_checkpoint"]
    module = build_training_module(config)
    attention_layers = [
        child for child in module.modules() if isinstance(child, WindowAttention2DTime)
    ]
    assert len(attention_layers) == 22
    assert not any("TemporalTTT" in type(child).__name__ for child in module.modules())
    assert not any(hasattr(child, "persistent_state") for child in module.modules())
    assert sum(parameter.numel() for parameter in module.parameters()) == 33_190_328
...
def test_clean_plain_checkpoint_schema() -> None:
    config = load_config(CONFIG)
    rollout_module = build_training_module(config)
    single_step_module = SingleStepSupervised(
        model=rollout_module.model,
        image_key=0,
        optimizer="adamw",
        monitor="val/loss_epoch",
    )
    single_step_state = single_step_module.state_dict()
    rollout_state = rollout_module.state_dict()
    assert single_step_state.keys() == rollout_state.keys()
    incompatible = rollout_module.load_state_dict(single_step_state, strict=True)
...
if __name__ == "__main__":
    ...
    print("PASS: clean Plain config and 22 Attention mixers")
    test_chunk_keeps_prediction_graph()
    print("PASS: four-step prediction graph")
    test_clean_plain_checkpoint_schema()
    print("PASS: clean Plain checkpoint schema")
